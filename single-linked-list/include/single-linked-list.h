...
public:
    ...
    SingleLinkedList(std::initializer_list<Type> values) {
        CreateFromIter(values.begin(), values.end());    
    }

    SingleLinkedList(const SingleLinkedList& other) {
        CreateFromIter(other.begin(), other.end());
    }
...
private:
    ...
    void CreateFromIter(Iterator it_begin, Iterator it_end)
    {
        size_t size = std::distance(it_begin, it_end);
        if(size == 0u)
        {
            return;
        }
